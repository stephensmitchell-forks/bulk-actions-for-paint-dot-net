		...
		public void Process(UpdateContext ctx, Dictionary<string, FileTypeSaveTokenPair> saveTokens, IProcessingEventListener events, int index)
		{
			...
			if(doc.Width != newWidth || doc.Height != newHeight)
			{
				...
				for(int i = 0; i < doc.Layers.Count; ++i)
				{
					...
					PaintDotNet.Rendering.IRenderer<PaintDotNet.ColorBgra> parallelRenderer = Environment.ProcessorCount > 1 ? Util.CreateParallelRenderer(observableRenderer, 7, 0) : Util.CreateTileizeRenderer(renderer, new PaintDotNet.Rendering.Int32Size(1 << 7, 1 << 7));

					PaintDotNet.Surface newSurface = new PaintDotNet.Surface(newWidth, newHeight);
					parallelRenderer.Render(newSurface, new PaintDotNet.Rendering.Int32Point(0, 0));
